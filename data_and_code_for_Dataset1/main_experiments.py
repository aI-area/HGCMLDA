...
def train_epoch(model, train_data, optim, opt):
    ...
    G_lnc_Kn = ConstructHW.constructHW_knn(concat_lnc_tensor.detach().cpu().numpy(), K_neigs=[11], is_probH=False)
    ...
    G_dis_Kn = ConstructHW.constructHW_knn(concat_dis_tensor.detach().cpu().numpy(), K_neigs=[11], is_probH=False)
