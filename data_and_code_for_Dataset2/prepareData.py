...
def prepare_data(opt):
    ...
    if int(cross_one_index.size(0) % opt.validation)!=0:
        for i in range(opt.validation):
            ...
            b=[s for s in range(opt.validation)]
            if i == opt.validation - 1:
                ...
                del a[i:i+2]
                del b[i]
                dataset['ld'].append({'test': [t.cat([new_one_index[j] for j in [-2,-1]]),new_zero_index[i]],
                              'train': [t.cat([new_one_index[j] for j in a]), t.cat([new_zero_index[j] for j in b])],'train_adj':temp1})
            else:
                temp1=dataset['ld_p'].clone()
